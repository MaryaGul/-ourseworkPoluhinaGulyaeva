...
def welcome(message):
    ...
    bot.send_message(message.chat.id,"Привет, {0.first_name}!\nМеня зовут '{1.first_name}'. \nЯ создан Линдой и Машей, чтобы играть с тобой в футбольные клубы :)".format(message.from_user, bot.get_me()), reply_markup=markup)
...
def lalala(message):
    if message.text != 'Начать игру' and message.text != 'Закончить игру' and (
        ...
        bot.send_message(message.chat.id, "Я не знаю такого футбольного клуба. Можешь назвать другой?")

    elif message.text != 'Начать игру' and message.text != 'Закончить игру' and (
            Was(message.text + '\n', message.chat.id) == 1 or Was(message.text, message.chat.id) == 1):
        bot.send_message(message.chat.id, "Такой клуб уже был. Можешь назвать другой?")
